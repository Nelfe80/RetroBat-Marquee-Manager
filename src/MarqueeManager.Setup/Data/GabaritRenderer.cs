...
public static class GabaritRenderer
{
    ...
    /// <summary>Deletes every cached gabarit render of a surface (call after an edit
    /// so the next view regenerates).</summary>
    public static void InvalidateSurface(string pluginRoot, string categoryRoot, string surfaceId)
    {
        var dir = Path.Combine(pluginRoot, "media", categoryRoot, ".cache", "surfaces", Safe(surfaceId), "systems");
