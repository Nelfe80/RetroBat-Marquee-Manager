...
namespace RetroBatMarqueeManager.Application.Services
{
    ...
    public class LayManager : IDisposable
    {
        ...
        /// </summary>
        public void LoadMameLayout(MameLayout layout, string layDir, string? romName = null)
        {
            // Debounce: skip if same ROM is already loaded (fired by .raw + enriched events)
            if (romName != null && romName == _lastLoadedRom && _lcdLayActive)
            {
            ...
            if (_config.LayEnabled && _config.LayLcdEnabled)
            {
            ...
            // ── Topper / ICCard: same direct path via MarqueeController ──
            if (_config.LayEnabled && _config.LayLcdEnabled)
            {
                if (layout.Views.ContainsKey("Topper_Only"))
                {
                    _marquee.LoadMameLayout(layout, "Topper_Only");
                    _logger.LogInformation("[LayManager] Topper .lay loaded.");
                }
            }
            ...
            if (_config.LayEnabled && _config.LayDmdEnabled && _config.DmdEnabled)
            {
                if (layout.Views.TryGetValue("DMD_Only", out var dmdView))
                {
                    ...
                    var adapter    = new DmdOutputAdapter(_dmdWrapper,
                    ...
                    if (dmdPipeline != null)
                    {
                        ...
                        _dmdService.SetExternalControl(true);
        ...
        public void Clear()
        {
            ...
            _pipelines.Clear();
            _dmdService.SetExternalControl(false);
            _currentLayout = null;
            _lastLoadedRom = null;
        }
        ...
        private ILayOutputAdapter CreateAdapter(string target) => target switch
        {
            ...
            "dmd"    => new DmdOutputAdapter(_dmdWrapper,
                                              _config.DmdWidth, _config.DmdHeight,
                                              _logger, LayScaleMode.Fit),
