    ...
    public class DmdDeviceWrapper : IDisposable
    {
        ...
        public bool PushHardwareCalibration(bool isHd, int brightness = -1, int usbPackageSizeOverride = -1, int refreshRateOverride = -1)
        {
            ...
            try
            {
                ...
                var needsUsbUpdate = !currentUsbPkg.HasValue || currentUsbPkg.Value != usbPkg;
                var needsRefreshUpdate = refreshRateOverride > 0;
                var needsUpscalingUpdate = isHd;
                var needsBrightnessUpdate = brightness >= 0 && brightness <= 15;

                if (!needsUsbUpdate && !needsRefreshUpdate && !needsUpscalingUpdate && !needsBrightnessUpdate)
                {
                    _logger.LogInformation($"[ZeDMD] UsbPackageSize already {usbPkg}; no update needed.");
                    _logger.LogInformation("[ZeDMD] Hardware calibration already current; skipping ClearScreen/SaveSettings/Reset.");
                    return false;
                }

                _zClear?.Invoke(_zedmdInstance);
                _logger.LogInformation("[ZeDMD] ClearScreen sent before firmware settings update.");
        ...
        public void HwClose()
        {
            if (_zedmdInstance != IntPtr.Zero && _zClose != null)
            {
                ...
                _logger.LogInformation("[ZeDMD] HwClose.");
            }
        }
